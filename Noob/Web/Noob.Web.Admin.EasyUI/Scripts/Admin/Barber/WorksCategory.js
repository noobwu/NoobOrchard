...
function openDialog(id, pid) {
    if (id) {
        addDialog({ url: rootUrl + "Barber/WorksCategory/Edit?id=" + id, title: '修改理发师作品类别', width: 360, height: 260 });
    }
    else {
        addDialog({ url: rootUrl + "Barber/WorksCategory/Create?pid=" + (pid ? pid : '0'), title: '添加理发师作品类别', width: 360, height: 260 });
    }
}
...
function updateStatus(status, id) {
    ...
    $.messager.confirm('确认', title, function (r) {
        if (r) {
            $.ajax({
                type: "POST",
                url: rootUrl + "Barber/WorksCategory/UpdateStatus",
...
function createModuleChild() {
    ...
    if (node) {
        ...
        $.messager.alert('提示信息', '请选择要在哪个节点添加理发师作品类别', 'info');
        return;
    }
}
//编辑菜单
function updateModule() {
    ...
    if (node) {
        ...
        $.messager.alert('提示信息', '请选择要编辑的理发师作品类别', 'info');
        return;
    }
}
//删除菜单
function deleteModule() {
    ...
    if (id != null && id != "") {
        ...
        $.messager.alert('提示信息', '请选择要删除的理发师作品类别', 'error');
        return;
    }
}
$(function () {
    $('#dg').treegrid({
        title: '理发师作品类别',
        url: rootUrl + 'Barber/WorksCategory/GetList',
        fitColumns: true,
        fit: true,
        idField: "id",
        treeField: 'name',
...
function deleteData(id) {
    $.messager.confirm('确认', '您确认想要删除吗？', function (r) {
        if (r) {
            $.ajax({
                type: "POST",
                url: rootUrl + "Barber/WorksCategory/Delete",
                data: "id=" + id,
                success: function (data) {
                    if (data.Code != undefined) {
                        if (data.Code == 1) {
