...
namespace Orchard.Queues
{
    public class InMemoryQueue<T> : QueueBase<T, InMemoryQueueOptions<T>> where T : class
    {
        ...
        protected override async Task<string> EnqueueImplAsync(T data)
        {
            ...
            _logger.DebugFormat("Queue {0} enqueue item: {1}", _options.Name, id);
            ...
            _logger.Debug("Enqueue: Set Event");
        ...
        protected override void StartWorkingImpl(Func<IQueueEntry<T>, CancellationToken, Task> handler, bool autoComplete, CancellationToken cancellationToken)
        {
            ...
            _logger.DebugFormat("Queue {0} start working", _options.Name);
            var linkedCancellationToken = GetLinkedDisposableCanncellationToken(cancellationToken);

            Task.Run(async () =>
            {
                _logger.DebugFormat("WorkerLoop Start {0}", _options.Name);

                while (!linkedCancellationToken.IsCancellationRequested)
                {
                    _logger.DebugFormat("WorkerLoop Signaled {0}", _options.Name);
                    ...
                    {
                        _logger.Error("Error on Dequeue: " + ex.Message, ex);
                    }
        ...
        protected override async Task<IQueueEntry<T>> DequeueImplAsync(CancellationToken linkedCancellationToken)
        {
            ...
            _logger.DebugFormat("Queue count: {0}", _queue.Count);
            ...
            _logger.Debug("Dequeue: Got Item");
        ...
        public override async Task AbandonAsync(IQueueEntry<T> entry)
        {
            ...
            if (info.Attempts < _options.Retries + 1)
            {
                ...
                {
                    _logger.DebugFormat("Adding item to wait list for future retry: {0}", entry.Id);
                    var unawaited = Run.DelayedAsync(GetRetryDelay(info.Attempts), () => RetryAsync(info));
                }
                else
                {
                    _logger.DebugFormat("Adding item back to queue for retry: {0}", entry.Id);
                    var unawaited = Task.Run(() => RetryAsync(info));
                }
            }
            else
            {
                _logger.DebugFormat("Exceeded retry limit moving to deadletter: {0}", entry.Id);
        ...
        {
            _logger.DebugFormat("Queue {0} retrying item: {1} Attempts: {2}", _options.Name, entry.Id, entry.Attempts);
            _queue.Enqueue(entry);
            _autoResetEvent.Set();
            return Task.CompletedTask;
        }
        ...
        protected override async Task<DateTime?> DoMaintenanceAsync()
        {
            ...
            try
            {
                foreach (var entry in _dequeued.Values.ToList())
                {
                    ...
                    if (abandonAt < utcNow)
                    {
                        _logger.Info($"DoMaintenance Abandon: {entry.Id}");
            ...
            {
                _logger.Error("DoMaintenance Error: " + ex.Message,ex);
            }
