...
namespace Orchard.Environment.Configuration {
    ...
    public class ShellSettingsManager : Component, IShellSettingsManager {
        ...
        IEnumerable<ShellSettings> IShellSettingsManager.LoadSettings() {
            Logger.Info("Reading ShellSettings...");
            var settingsList = LoadSettingsInternal().ToArray();

            var tenantNamesQuery =
                from settings in settingsList
                select settings.Name;
            Logger.InfoFormat("Returning {0} ShellSettings objects for tenants {1}.", tenantNamesQuery.Count(), String.Join(", ", tenantNamesQuery));
        ...
        void IShellSettingsManager.SaveSettings(ShellSettings settings) {
            ...
            Logger.InfoFormat("Saving ShellSettings for tenant '{0}'...", settings.Name);
            ...
            Logger.InfoFormat("ShellSettings saved successfully; flagging tenant '{0}' for restart.", settings.Name);
