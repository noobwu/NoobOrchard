...
namespace Orchard.Metrics
{
    public abstract class CacheBucketMetricsClientBase : BufferedMetricsClientBase, IMetricsClientStats {
        ...
        private async Task StoreCounterAsync(TimeBucket timeBucket, AggregatedCounterMetric counter) {
          _logger.Debug($"Storing counter name={counter.Key.Name} value={counter.Value} time={counter.Key.Duration}");
          ...
          _logger.Debug($"Done storing counter name={counter.Key.Name}");
        }

        private async Task StoreGaugeAsync(TimeBucket timeBucket, AggregatedGaugeMetric gauge) {
          _logger.Debug($"Storing gauge name={gauge.Key.Name} count={gauge.Count} total={gauge.Total} last={gauge.Last} min={gauge.Min} max={gauge.Max} time={gauge.Key.StartTimeUtc}");
          ...
          _logger.Debug($"Done storing gauge name={gauge.Key.Name}");
        }

        private async Task StoreTimingAsync(TimeBucket timeBucket, AggregatedTimingMetric timing) {
          _logger.Debug($"Storing timing name={timing.Key.Name} count={timing.Count} total={timing.TotalDuration} min={timing.MinDuration} max={timing.MaxDuration} time={timing.Key.StartTimeUtc}");
          ...
          _logger.Debug($"Done storing timing name={timing.Key.Name}");
        }
