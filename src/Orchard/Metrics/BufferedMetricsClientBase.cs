...
namespace Orchard.Metrics
{
    ...
    public abstract class BufferedMetricsClientBase : IBufferedMetricsClient {
        ...
        public BufferedMetricsClientBase(MetricsClientOptionsBase options) {
            ...
            _logger = options.LoggerFactory.GetLogger(GetType());
            if (options.Buffered)
        ...
        private void OnMetricsTimer(object state) {
            try {
                ...
                _logger.Error($"Error flushing metrics: {ex.Message}", ex);
            }
        }
        ...
        public async Task FlushAsync() {
            if (_sendingMetrics || _queue.IsEmpty)
                return;
            _logger.Debug($"Flushing metrics: count={ _queue.Count}");
            ...
            try {
               ...
               _logger.Debug($"Dequeued {entries.Count} metrics");
            ...
            foreach (var timeBucket in _timeBuckets) {
                try {
                    ...
                    if (metrics.Count > 1 && counters.Count > 0)
                       _logger.Debug($"Aggregated {counters.Count} counter(s) into {groupedCounters.Count} counter group(s)");
                    ...
                    if (metrics.Count > 1 && gauges.Count > 0)
                       _logger.Debug($"Aggregated {gauges.Count} gauge(s) into {groupedGauges.Count} gauge group(s)");
                    ...
                    if (metrics.Count > 1 && timings.Count > 0)
                       _logger.Debug($"Aggregated {timings.Count} timing(s) into {groupedTimings.Count} timing group(s)");
                    ...
                    _logger.Error($"Error aggregating metrics: {ex.Message}",ex);
                    throw;
                }
            }
        }
        ...
        private async Task StoreAggregatedMetricsInternalAsync(TimeBucket timeBucket, ICollection<AggregatedCounterMetric> counters, ICollection<AggregatedGaugeMetric> gauges, ICollection<AggregatedTimingMetric> timings) {
           _logger.Debug($"Storing {counters.Count} counters, {gauges.Count} gauges, {timings.Count} timings.");

            try {
                await Run.WithRetriesAsync(() => StoreAggregatedMetricsAsync(timeBucket, counters, gauges, timings)).AnyContext();
            } catch (Exception ex) {
                _logger.Error($"Error storing aggregated metrics: {ex.Message}",ex);
                throw;
            }

            await OnCountedAsync(counters.Sum(c => c.Value)).AnyContext();
           _logger.Debug("Done storing aggregated metrics");
        }
        ...
        public async Task<bool> WaitForCounterAsync(string statName, Func<Task> work, long count = 1, CancellationToken cancellationToken = default(CancellationToken)) {
            ...
            using (Counted.AddHandler((s, e) => {
               ...
               _logger.Debug($"Wait: count={currentCount}");
                ...
                do {
                   ...
                   _logger.Debug($"Got signal: count={currentCount} expected={count}");
                } while (cancellationToken.IsCancellationRequested == false && currentCount > 0);
            }

           _logger.Debug($"Done waiting: count={currentCount} expected={count} success={ currentCount <= 0} time={DateTime.UtcNow.Subtract(start)}" );
