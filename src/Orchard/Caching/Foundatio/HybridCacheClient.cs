...
namespace Orchard.Caching
{
    ...
    public class HybridCacheClient : IHybridCacheClient {
        ...
        public HybridCacheClient(ICacheClient distributedCacheClient, IMessageBus messageBus, ILoggerFactory loggerFactory = null) {
            _logger = loggerFactory.GetLogger(typeof(HybridCacheClient));
        ...
        private async Task OnLocalCacheItemExpiredAsync(object sender, ItemExpiredEventArgs args) {
            ...
            _logger.DebugFormat("Local cache expired event: key={0}", args.Key);
        ...
        private async Task OnRemoteCacheItemExpiredAsync(InvalidateCache message) {
            ...
            _logger.DebugFormat(String.Format("Invalidating local cache from remote: id={0} expired={1} keys={2}", message.CacheId, message.Expired, String.Join(",", message.Keys ?? new string[] { })));
            Interlocked.Increment(ref _invalidateCacheCalls);
            if (message.FlushAll) {
                await _localCache.RemoveAllAsync().AnyContext();
                _logger.Debug("Fushed local cache");
                ...
                _logger.DebugFormat("Removed {0} keys from local cache", results);
            } else {
        ...
        public async Task<CacheValue<T>> GetAsync<T>(string key) {
            ...
            if (cacheValue.HasValue) {
                _logger.DebugFormat("Local cache hit: {0}", key);
                Interlocked.Increment(ref _localCacheHits);
                return cacheValue;
            }

            _logger.DebugFormat("Local cache miss: {0}", key);
            ...
            if (cacheValue.HasValue) {
                ...
                _logger.DebugFormat("Setting Local cache key: {0} with expiration: {1}", key, expiration);
        ...
        public async Task<bool> AddAsync<T>(string key, T value, TimeSpan? expiresIn = null) {
            _logger.DebugFormat("Adding key \"{0}\" to local cache with expiration: {1}", key, expiresIn);
        ...
        public async Task<bool> SetAsync<T>(string key, T value, TimeSpan? expiresIn = null) {
            _logger.DebugFormat("Setting key \"{0}\" to local cache with expiration: {1}", key, expiresIn);
        ...
        public async Task<int> SetAllAsync<T>(IDictionary<string, T> values, TimeSpan? expiresIn = null) {
            ...
            _logger.DebugFormat("Adding keys \"{0}\" to local cache with expiration: {1}", values.Keys, expiresIn);
