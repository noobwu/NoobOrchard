...
namespace Orchard.Messaging.RabbitMq
{
    ...
    {
        private static readonly ILogger Log = LoggerManager.GetLogger(typeof(RabbitMqWorker));
        ...
        public virtual void Start()
        {
            ...
            {
                Log.Debug(string.Format("Starting MQ Handler Worker: {0}...",QueueName));

                //Should only be 1 thread past this point
                bgThread = new Thread(Run)
                {
                    Name = string.Format("{0}: {1}",GetType().Name, QueueName),
                    IsBackground = true,
                };
                bgThread.Start();
            }
        ...
        private void Run()
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Log.Warn(string.Format("Received {0} in Worker: {1}",ex.GetType().Name, QueueName));
                    return;
                }
#endif
        ...
        private IModel WaitForReconnect()
        {
            ...
            while (true)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Log.Debug(string.Format("Retrying to Reconnect after {0}ms...",waitMs), ex);
                    Thread.Sleep(waitMs);
                }
            }
        }
        ...
        private RabbitMqBasicConsumer WaitForReconnectSubscription()
        {
            ...
            while (true)
            {
                ...
                {
                    var waitMs = Math.Min(retries++ * 100, 10000);
                    Log.Debug(string.Format("Retrying to Reconnect Subscription after {0}ms...",waitMs), ex);
                    Thread.Sleep(waitMs);
                }
            }
        }
        ...
        public virtual void Stop()
        {
            ...
            {
                Log.Debug(string.Format("Stopping Rabbit MQ Handler Worker: {0}...",QueueName));
                if (mqFactory.UsePolling)
                {
                    Thread.Sleep(100);
                    lock (msgLock)
        ...
        public virtual string GetStatus()
        {
            return string.Format("[Worker: {0}, Status: {1}, ThreadStatus: {2}, LastMsgAt: {3}]",
                QueueName, WorkerStatus.ToString(status), bgThread.ThreadState, LastMsgProcessed);
        }
    }
}