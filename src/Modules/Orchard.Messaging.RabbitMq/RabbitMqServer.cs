...
namespace Orchard.Messaging.RabbitMq
{
    ...
    {
        private static readonly ILogger Log = LoggerManager.GetLogger(typeof(RabbitMqServer));
        ...
        {
            Init(new RabbitMqMessageFactory(connectionString, username, password,virtualHost));
        }
        ...
        public virtual void Init()
        {
            ...
            using (var channel = Connection.OpenChannel())
            {
                ...
                for (var i = 0; i < workers.Length; i++)
                {
                    ...
                    if (!queueWorkerIndexMap.TryGetValue(worker.QueueName, out int[] workerIds))
                    {
        ...
        void WorkerErrorHandler(RabbitMqWorker source, Exception ex)
        {
            ...
            for (int i = 0; i < workers.Length; i++)
            {
                ...
                if (worker == source)
                {
                    Log.Debug(string.Format("Starting new {0} Worker at index {1}...",source.QueueName, i));
