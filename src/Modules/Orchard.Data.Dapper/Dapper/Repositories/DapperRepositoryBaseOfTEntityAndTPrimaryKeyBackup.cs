...
namespace Orchard.Data.Dapper.Repositories
{
    ...
    public class DapperRepositoryBaseBackup<TEntity, TPrimaryKey> :
        RepositoryBase<TEntity, TPrimaryKey>
        where TEntity : class, IEntity<TPrimaryKey>
    {
        public IActiveTransactionProvider ActiveTransactionProvider { get; private set; }
        private DbConnection Connection
        {
            get { return ActiveTransactionProvider.GetActiveConnection(ActiveTransactionProviderArgs.Empty); }
        }
        ...
        {
            get { return ActiveTransactionProvider.GetActiveTransaction(ActiveTransactionProviderArgs.Empty); }
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        public override TEntity Single(Expression<Func<TEntity, bool>> predicate)
        {
            ...
            return Connection.GetList<TEntity>(predicateGroup).FirstOrDefault();
        }
        ...
        {
            IPredicate predicateGroup = predicate.ToPredicateGroup<TEntity, TPrimaryKey>(DapperQueryFilterExecuter);
            var result = await Connection.GetListAsync<TEntity>(predicateGroup);
        ...
        public override IQueryable<TEntity> GetAllIncluding(params Expression<Func<TEntity, object>>[] propertySelectors)
        {
            ...
            var list = Connection.GetList<TEntity>(predicateGroup);
        ...
        {
            IPredicate predicateGroup = predicate.ToPredicateGroup<TEntity, TPrimaryKey>(DapperQueryFilterExecuter);
            return Connection.GetList<TEntity>(predicateGroup, sort: GetSortable(orderByExpressions)).ToList();
        }
        /// <summary>
        /// Used to get all entities based on given <paramref name="predicate"/>.
        /// </summary>
        ...
        public override async Task<List<TEntity>> GetListAsync(Expression<Func<TEntity, bool>> predicate, params IOrderByExpression<TEntity>[] orderByExpressions)
        {
            ...
            IPredicate predicateGroup = predicate.ToPredicateGroup<TEntity, TPrimaryKey>(DapperQueryFilterExecuter);
            var result = await Connection.GetListAsync<TEntity>(predicateGroup, sort: GetSortable(orderByExpressions));
            return result.ToList();
        }
        ...
        {
            TPrimaryKey primaryKey = await Connection.InsertAsync(entity, ActiveTransaction);
            if (entity.IsTransient())
            {
                entity.Id = primaryKey;
            }
        ...
        {
            await Connection.InsertAsync(entities, ActiveTransaction);
            return entities.Count();
        }
        #endregion
        ...
        {
            await Connection.UpdateAsync(entity, ActiveTransaction);
            return entity;
        }
        ...
        {
            return Connection.Delete(entity, ActiveTransaction) ? 1 : 0;
        }
        ...
        {
            return await Connection.DeleteAsync(entity, ActiveTransaction) ? 1 : 0;
        }
        ...
        public override int Delete(TPrimaryKey id)
        {
            ...
            return Connection.Delete<TEntity>(predicateGroup, ActiveTransaction) ? 1 : 0;
        }
        ...
        public override async Task<int> DeleteAsync(TPrimaryKey id)
        {
            Expression<Func<TEntity, bool>> predicate = CreateEqualityExpressionForId(id);
            IPredicate predicateGroup = predicate.ToPredicateGroup<TEntity, TPrimaryKey>();
            return await Connection.DeleteAsync<TEntity>(predicateGroup, ActiveTransaction) ? 1 : 0;
        }
        ...
        {
            IPredicate predicateGroup = DapperQueryFilterExecuter.ExecuteFilter<TEntity, TPrimaryKey>();
            return await Connection.CountAsync<TEntity>(predicateGroup);
        }
        ...
        {
            IPredicate predicateGroup = DapperQueryFilterExecuter.ExecuteFilter<TEntity, TPrimaryKey>();
            var result = await Connection.CountAsync<TEntity>(predicateGroup);
            return result;
        }
        ...
        public override List<TEntity> GetPaggingList(Expression<Func<TEntity, bool>> predicate, int pageIndex, int pageSize, IOrderByExpression<TEntity>[] orderByExpressions)
        {
            ...
            IPredicate predicateGroup = predicate.ToPredicateGroup<TEntity, TPrimaryKey>(DapperQueryFilterExecuter);
            return Connection.GetPage<TEntity>(predicateGroup, GetSortable(orderByExpressions), pageIndex, pageSize).ToList();
        }
        /// <summary>
        ///     Gets the list paged.
        /// </summary>
        ...
        public override async Task<List<TEntity>> GetPaggingListAsync(Expression<Func<TEntity, bool>> predicate, int pageIndex, int pageSize, IOrderByExpression<TEntity>[] orderByExpressions)
        {
            ...
            pageIndex = pageIndex - 1;
            IPredicate predicateGroup = predicate.ToPredicateGroup<TEntity, TPrimaryKey>(DapperQueryFilterExecuter);
            var result = await Connection.GetPageAsync<TEntity>(predicateGroup, GetSortable(orderByExpressions), pageIndex, pageSize);
            return result.ToList();
        }
        /// <summary>
        ...
        {
            return await Connection.QueryAsync<TEntity>(query, parameters, ActiveTransaction);
        }
        ...
        {
            return await Connection.QueryAsync<TAny>(query, parameters, ActiveTransaction);
        }
        ...
        {
            IPredicate predicateGroup = predicate.ToPredicateGroup<TEntity, TPrimaryKey>(DapperQueryFilterExecuter);
            return Connection.GetSet<TEntity>(predicateGroup, sortingExpression.ToSortable(ascending), firstResult, maxResults, ActiveTransaction);
        }
