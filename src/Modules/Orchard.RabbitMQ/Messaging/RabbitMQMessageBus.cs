...
namespace Orchard.RabbitMQ.Messaging
{
    ...
    public class RabbitMQMessageBus : MessageBusBase<RabbitMQMessageBusOptions> {
        ...
        public RabbitMQMessageBus(RabbitMQMessageBusOptions options) : base(options) {
            ...
            Uri uri = new Uri(options.ConnectionString);
            _factory = new ConnectionFactory { Uri = uri, AutomaticRecoveryEnabled = true,VirtualHost=options.VirtualHost };
        }
        ...
        protected override async Task EnsureTopicSubscriptionAsync(CancellationToken cancellationToken) {
            ...
            using (await _lock.LockAsync().AnyContext()) {
                ...
                _logger.Debug($"The unique channel number for the subscriber is : { _subscriberChannel.ChannelNumber}");
            }
        }
        ...
        private async void OnMessageAsync(object sender, BasicDeliverEventArgs e) {
            ...
            _logger.Debug($"OnMessageAsync({ e.BasicProperties?.MessageId})");
            MessageBusData message;
            try {
                message = await _serializer.DeserializeAsync<MessageBusData>(e.Body).AnyContext();
            } catch (Exception ex) {
                _logger.ErrorFormat(ex, "OnMessageAsync({0}) Error deserializing messsage: {1}", e.BasicProperties?.MessageId, ex.Message);
                return;
            }
        ...
        protected override async Task EnsureTopicCreatedAsync(CancellationToken cancellationToken) {
            ...
            using (await _lock.LockAsync().AnyContext()) {
                ...
                _logger.Debug($"The unique channel number for the publisher is : { _publisherChannel.ChannelNumber}");
            }
        }
        ...
        private bool CreateDelayedExchange(IModel model) {
            ...
            try {
                ...
                if (o.ShutdownReason.ReplyCode == 503) {
                    ...
                    success = false;
                    _logger.Info("Not able to create x-delayed-type exchange",o);
                }
            }
