...
namespace Orchard.Data.EntityFramework.Repositories
{
        ...
        where TEntity : class, IEntity<TPrimaryKey>
    {
        ...
        /// </summary>
        public virtual EfDbContext Context { get; private set; }
        ...
        /// <summary>
        /// Creates a new <see cref="EfRepositoryBase{TDbContext,TEntity,TPrimaryKey}"/> object.
        /// </summary>
        ...
        {
            //return await Task.FromResult(ApplyOrderBy(GetAll().Where(predicate),orderByExpressions).ToList());
        ...
        {
            var result = Table.AddRange(entities);
            Context.SaveChanges();
            return result.Count();
        }
        ...
        {
            return ApplyFilters(Context.Set<TEntity>()).Count();
        }
        ...
        {
            return await ApplyFilters(Context.Set<TEntity>()).CountAsync();
        }
        ...
        {
            int result = await GetAll().CountAsync(predicate);
            return result;
        }
        ...
        {
            //在EfDbContext的OnModelCreating过滤
        ...
        private IQueryable<TEntity> ApplySoftDeleteFilter(IQueryable<TEntity> query)
        {
            if (typeof(ISoftDelete).GetTypeInfo().IsAssignableFrom(typeof(TEntity)))
            {
                ...
                query = query.Where(FilterSoftDeleteExpression());
