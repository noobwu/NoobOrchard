...
namespace Orchard.Data.NHibernate.Tests
{
    ...
    public abstract class NHibernateTestsBase
    {
        ...
        public virtual void Init()
        {
            ...
            fluentConfiguration.Database(MsSqlConfiguration.MsSql2008.ConnectionString(
                x => x.Server(".")
                      .Database("Test")
                      .Username("sa")
                      .Password("123456")
              ));
            fluentConfiguration.Mappings(m => m.FluentMappings.AddFromAssembly(Assembly.GetExecutingAssembly()));
            _sessionFactory = fluentConfiguration.BuildSessionFactory();
            _session = _sessionFactory.OpenSession();
            var builder = new ContainerBuilder();
            builder.RegisterInstance(_sessionFactory);
            builder.Register(c => _sessionFactory).SingleInstance();
