    ...
    public abstract class CacheClientTestsBase : TestWithLoggingBase
    {
        ...
        public virtual async Task CanGetAllAsync()
        {
            ...
            using (cache)
            {
                ...
                Assert.AreEqual(1, result["test1"].Value);
                Assert.AreEqual(2, result["test2"].Value);
                Assert.AreEqual(3, result["test3"].Value);
        ...
        public virtual async Task CanGetAllWithOverlapAsync()
        {
            ...
            using (cache)
            {
                ...
                Assert.AreEqual(1.0, result["test1"].Value);
                Assert.AreEqual(2.0, result["test2"].Value);
                Assert.AreEqual(3.5, result["test3"].Value);
                Assert.AreEqual(4.0, result["test4"].Value);
                Assert.AreEqual(5.0, result["test5"].Value);
            }
        }
        ...
        public virtual async Task CanSetAsync()
        {
            ...
            using (cache)
            {
                ...
                Assert.AreEqual(3, await cache.SetAddAsync("set", new List<int> { 1, 1, 2, 3 }));
        ...
        public virtual async Task CanSetAndGetValueAsync()
        {
            ...
            using (cache)
            {
                ...
                Assert.AreEqual(1, (await cache.GetAsync<int>("test")).Value);

                Assert.False(await cache.AddAsync("test", 2));
                Assert.AreEqual(1, (await cache.GetAsync<int>("test")).Value);

                Assert.True(await cache.ReplaceAsync("test", 2));
                Assert.AreEqual(2, (await cache.GetAsync<int>("test")).Value);
                ...
                Assert.AreEqual("Testing", result.Value.Message);
            }
        }
        ...
        public virtual async Task CanAddAsync()
        {
            ...
            using (cache)
            {
                ...
                Assert.AreEqual(val, (await cache.GetAsync<string>(key)).Value);
                ...
                Assert.AreEqual("nested", (await cache.GetAsync<string>(key + ":1")).Value);
            }
        }
        ...
        public virtual async Task CanTryGetAsync()
        {
            ...
            using (cache)
            {
                ...
                Assert.AreEqual(1L, cacheValue.Value);
                ...
                Assert.True(cacheValue.HasValue);
                Assert.AreEqual(Int64.MaxValue, cacheValue.Value);
        ...
        public virtual async Task CanUseScopedCachesAsync()
        {
            ...
            using (cache)
            {
                ...
                Assert.AreEqual(1, (await cache.GetAsync<int>("test")).Value);
                Assert.AreEqual(2, (await scopedCache1.GetAsync<int>("test")).Value);
                Assert.AreEqual(3, (await nestedScopedCache1.GetAsync<int>("test")).Value);

                Assert.AreEqual(3, (await scopedCache1.GetAsync<int>("nested:test")).Value);
                Assert.AreEqual(3, (await cache.GetAsync<int>("scoped1:nested:test")).Value);

                // ensure GetAllAsync returns unscoped keys
                Assert.AreEqual("test", (await scopedCache1.GetAllAsync<int>("test")).Keys.FirstOrDefault());
                Assert.AreEqual("test", (await nestedScopedCache1.GetAllAsync<int>("test")).Keys.FirstOrDefault());
                ...
                Assert.AreEqual(1, (await cache.GetAsync<int>("test")).Value);
                Assert.AreEqual(1, (await scopedCache2.GetAsync<int>("test")).Value);
                ...
                Assert.AreEqual(0, await scopedCache1.GetAsync<double>("total", 0));
                Assert.AreEqual(10, await scopedCache1.IncrementAsync("total", 10));
                Assert.AreEqual(10, await scopedCache1.GetAsync<double>("total", 0));

                Assert.AreEqual(0, await nestedScopedCache1.GetAsync<double>("total", 0));
                Assert.AreEqual(20, await nestedScopedCache1.IncrementAsync("total", 20));
                Assert.AreEqual(20, await nestedScopedCache1.GetAsync<double>("total", 0));
                Assert.AreEqual(1, await nestedScopedCache1.RemoveAllAsync(new[] { "id", "total" }));
                Assert.AreEqual(0, await nestedScopedCache1.GetAsync<double>("total", 0));

                Assert.AreEqual(1, await scopedCache1.RemoveAllAsync(new[] { "id", "total" }));
                Assert.AreEqual(0, await scopedCache1.GetAsync<double>("total", 0));
            }
        }
        ...
        public virtual async Task CanRemoveByPrefixAsync()
        {
            ...
            using (cache)
            {
                ...
                Assert.AreEqual(1, (await cache.GetAsync<int>(prefix + "test")).Value);
                Assert.AreEqual(1, (await cache.GetAsync<int>("test")).Value);

                Assert.AreEqual(0, await cache.RemoveByPrefixAsync(prefix + ":doesntexist"));
                Assert.AreEqual(2, await cache.RemoveByPrefixAsync(prefix));
                Assert.False((await cache.GetAsync<int>(prefix + "test")).HasValue);
                Assert.False((await cache.GetAsync<int>(prefix + "test2")).HasValue);
                Assert.AreEqual(1, (await cache.GetAsync<int>("test")).Value);

                Assert.AreEqual(1, await cache.RemoveByPrefixAsync(String.Empty));
            }
        }
        ...
        public virtual async Task CanSetAndGetObjectAsync()
        {
            ...
            using (cache)
            {
                ...
                Assert.AreEqual(dt, cachedValue.Value.Date);
                Assert.False(value.Equals(cachedValue.Value), "Should not be same reference object.");
                Assert.AreEqual("Hello World", cachedValue.Value.Message);
                Assert.AreEqual("test", cachedValue.Value.Type);
            }
        }
        ...
        public virtual async Task CanIncrementAsync()
        {
            ...
            using (cache)
            {
                ...
                Assert.AreEqual(1, await cache.IncrementAsync("test"));
                Assert.AreEqual(1, await cache.IncrementAsync("test1"));
                Assert.AreEqual(0, await cache.IncrementAsync("test3", 0));

                // The following is not supported by redis.
                if (cache is InMemoryCacheClient)
                {
                    Assert.True(await cache.SetAsync("test2", "stringValue"));
                    Assert.AreEqual(1, await cache.IncrementAsync("test2"));
                }
            }
        }
        ...
        public virtual async Task CanManageSetsAsync()
        {
            ...
            using (cache)
            {
                ...
                Assert.ThrowsAsync<ArgumentNullException>(() => cache.SetAddAsync(null, 1));
                Assert.ThrowsAsync<ArgumentNullException>(() => cache.SetAddAsync(String.Empty, 1));

                Assert.ThrowsAsync<ArgumentNullException>(() => cache.SetRemoveAsync(null, 1));
                Assert.ThrowsAsync<ArgumentNullException>(() => cache.SetRemoveAsync(String.Empty, 1));

                Assert.ThrowsAsync<ArgumentNullException>(() => cache.GetSetAsync<ICollection<int>>(null));
                Assert.ThrowsAsync<ArgumentNullException>(() => cache.GetSetAsync<ICollection<int>>(String.Empty));
        ...
        public virtual async Task MeasureThroughputAsync()
        {
            ...
            using (cache)
            {
                ...
                for (int i = 0; i < itemCount; i++)
                {
                    ...
                    Assert.AreEqual(13422, (await cache.GetAsync<int>("test")).Value);
        ...
        public virtual async Task MeasureSerializerSimpleThroughputAsync()
        {
            ...
            using (cache)
            {
                ...
                for (int i = 0; i < itemCount; i++)
                {
                    ...
                    Assert.AreEqual("Hello", model.Value.Data1);
                    Assert.AreEqual(12, model.Value.Data2);
                    await metrics.CounterAsync("work");
                }
