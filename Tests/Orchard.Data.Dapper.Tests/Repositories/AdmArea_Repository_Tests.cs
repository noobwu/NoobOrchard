...
namespace Orchard.Data.Dapper.Tests.Repositories
{
    public class AdmArea_Repository_Tests : DapperNUnitTestBase
    {
        protected override void Register(ContainerBuilder builder)
        {

        }
        private Domain.Repositories.IRepository<AdmArea,int> _admAreaRepository;
...
FROM "wt_adm_area"
        ...
        {
            /*exec sp_executesql N'SELECT [wt_adm_area].[Id], [wt_adm_area].[AreaID], [wt_adm_area].[AreaName], [wt_adm_area].[ParentId], [wt_adm_area].[ShortName], [wt_adm_area].[LevelType], [wt_adm_area].[CityCode], [wt_adm_area].[ZipCode], [wt_adm_area].[AreaNamePath], [wt_adm_area].[AreaIDPath], [wt_adm_area].[Lng], [wt_adm_area].[Lat], [wt_adm_area].[PinYin], [wt_adm_area].[ShortPinYin], [wt_adm_area].[PYFirstLetter], [wt_adm_area].[SortOrder], [wt_adm_area].[Status], [wt_adm_area].[CreateTime], [wt_adm_area].[CreateUser], [wt_adm_area].[UpdateTime], [wt_adm_area].[UpdateUser], [wt_adm_area].[DeleteFlag], [wt_adm_area].[DeleteTime], [wt_adm_area].[DeleteUser] FROM [wt_adm_area] WHERE (([wt_adm_area].[Id] >= @Id_0) AND ([wt_adm_area].[Id] < @Id_1)) 
             * ORDER BY [wt_adm_area].[AreaID] ASC, [wt_adm_area].[Id] DESC',N'@Id_0 int,@Id_1 int',@Id_0=100,@Id_1=105*/
        ...
        {
            /*exec sp_executesql N'SELECT TOP(3) [_proj].[Id], [_proj].[AreaID], [_proj].[AreaName], [_proj].[ParentId], [_proj].[ShortName], [_proj].[LevelType], [_proj].[CityCode], [_proj].[ZipCode], [_proj].[AreaNamePath], [_proj].[AreaIDPath], [_proj].[Lng], [_proj].[Lat], [_proj].[PinYin], [_proj].[ShortPinYin], [_proj].[PYFirstLetter], [_proj].[SortOrder], [_proj].[Status], [_proj].[CreateTime], [_proj].[CreateUser], [_proj].[UpdateTime], [_proj].[UpdateUser], [_proj].[DeleteFlag], [_proj].[DeleteTime], [_proj].[DeleteUser] FROM (SELECT ROW_NUMBER() OVER(ORDER BY [wt_adm_area].[AreaID] ASC, [wt_adm_area].[Id] DESC) AS [_row_number], [wt_adm_area].[Id], [wt_adm_area].[AreaID], [wt_adm_area].[AreaName], [wt_adm_area].[ParentId], [wt_adm_area].[ShortName], [wt_adm_area].[LevelType], [wt_adm_area].[CityCode], [wt_adm_area].[ZipCode], [wt_adm_area].[AreaNamePath], [wt_adm_area].[AreaIDPath], [wt_adm_area].[Lng], [wt_adm_area].[Lat], [wt_adm_area].[PinYin], [wt_adm_area].[ShortPinYin], [wt_adm_area].[PYFirstLetter], [wt_adm_area].[SortOrder], [wt_adm_area].[Status], [wt_adm_area].[CreateTime], [wt_adm_area].[CreateUser], [wt_adm_area].[UpdateTime], [wt_adm_area].[UpdateUser], [wt_adm_area].[DeleteFlag], [wt_adm_area].[DeleteTime], [wt_adm_area].[DeleteUser] FROM [wt_adm_area] WHERE (([wt_adm_area].[Id] >= @Id_0) AND ([wt_adm_area].[Id] < @Id_1))) [_proj] 
WHERE [_proj].[_row_number] >= @_pageStartRow 
ORDER BY [_proj].[_row_number]',N'@Id_0 int,@Id_1 int,@_pageStartRow int',@Id_0=100,@Id_1=105,@_pageStartRow=1*/
            Expression<Func<AdmArea, bool>> predicate = x => x.Id >= 100 && x.Id < 105;
            Action<Orderable<AdmArea>> orderAction = (o =>
            {
                o.Desc(x => x.AreaID).Asc(x => x.CreateUser);
...
',N'@0 varchar(50),@1 varchar(50),@2 varchar(50),@3 varchar(50),@4 tinyint,@5 varchar(50),@6 varchar(50),@7 nvarchar(500),@8 varchar(500),@9 decimal(18,2),@10 decimal(18,2),@11 varchar(20),@12 varchar(10),@13 int,@14 tinyint,@15 datetime2(7),@16 int,@17 datetime2(7),@18 int,@19 int',@0='409878',@1='测试地区名称259 Update',@2='100000',@3='地区简称',@4=0,@5='',@6='',@7=N'中国,测试地区名称259',@8='100000,409878',@9=0,@10=0,@11='',@12='',@13=92,@14=0,@15='2017-06-21 22:13:50.3718472',@16=8442,@17='2017-06-21 22:13:51.2108839',@18=1430,@19=3759
        ...
        {
            /*
            SELECT COUNT(*) AS [Total] FROM [wt_adm_area]
           */
...
FROM "wt_adm_area"
        ...
        {
            /*exec sp_executesql N'SELECT COUNT(*) AS [Total] FROM [wt_adm_area] WHERE (([wt_adm_area].[Id] > @Id_0) AND ([wt_adm_area].[Id] < @Id_1))',N'@Id_0 int,@Id_1 int',@Id_0=100,@Id_1=1000
    */
